...
// Shared alias for the authenticated Supabase user object.
// Keeps HomePage (and anywhere else that needs it) in sync
// with the exact type App() gets back from supabase.auth.getUser().
type AppUser = Awaited<
...
function getDaysRemaining(test: Test) {
  ...
  // Malformed/missing test dates should degrade gracefully
  // instead of producing NaN-based countdowns.
  if (!isValidDate(testDate)) return null
...
function formatTestDate(test: Test) {
  ...
  if (!isValidDate(testDate)) return "Date not set"
...
function formatTestTime(test: Test) {
  ...
  if (!isValidDate(testDate)) return "Time not set"
...
/* =========================================================
   ...
   Prevents a single malformed/unexpected value (bad date,
   missing field, unexpected shape from Supabase, etc.) from
   crashing the whole app to a blank screen. When nothing
   throws, this renders its children exactly as before and
...
> {
  ...
  componentDidCatch(error: unknown, info: unknown) {
    console.error("PreMed UI error:", error, info)
  }
      ...
      return (
        <div className="flex min-h-[50vh] flex-col items-center justify-center px-6 text-center">
          ...
          <p className="mt-2 max-w-sm text-xs leading-5 text-white/35">
            Try switching to another section from the sidebar,
            or refresh the page.
          </p>
        </div>
      )
...
function Sidebar({
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 border-r border-white/10 bg-[#090909] px-4 py-5 text-white">
...
function TestResultModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-2xl rounded-xl border border-white/10 bg-[#151515] p-6 shadow-2xl">
                ...
                {test.status === "completed" && (
                  ...
                  {test.status ===
                    ...
                    <span className="text-xs text-white/30">
                      {(() => {
                        const remaining = getDaysRemaining(test)
                        if (remaining === null) return "Date not set"
                        return remaining === 0
                          ? "Today"
                          : `${remaining}d`
                      })()}
...
function SyllabusUnitCard({
  ...
  setActivityState: Dispatch<
    SetStateAction<ActivityState>
  >
}) {
...
function formatCountdown(test: Test) {
  ...
  if (!isValidDate(target)) return "Date not set"
    ...
    <div className="min-h-screen bg-[#111111] text-white">
      ...
      <main className="ml-64 min-h-screen">
        <div className="mx-auto max-w-7xl px-10 py-10">
          <ErrorBoundary
            key={page}
            fallbackTitle={`Something went wrong loading ${page}.`}
          >
            {page === "Home" && (
              <HomePage
                nextTest={nextTest}
